        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('authordashboard.novels') }}">Mes romans</a></li>
                    <li class="breadcrumb-item active">{{ $novel->title }}</li>
                </ol>
            </nav>
            <div>
                <a href="{{ route('authordashboard.novels.read', $novel) }}" 
                   class="btn btn-success me-2">
                    <i class="bi bi-book"></i> Lire
                </a>

                <a href="{{ route('authordashboard.novels.chapters.create') }}" 
                   class="btn btn-primary me-2">
                    <i class="bi bi-plus"></i> Ajouter un chapitre
                </a>
                <a href="{{ route('authordashboard.novels.edit', $novel) }}" 
            ...
            <div class="col-md-4">
                ...
                <div class="stats-card mb-4">
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span class="fw-bold">{{ number_format($novel->view_count ?? 0) }}</span>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span><i class="bi bi-heart"></i> Likes</span>
                        <span class="fw-bold">{{ number_format($novel->like_count ?? 0) }}</span>
                    </div>
                    <div class="d-flex justify-content-between">
                        <span><i class="bi bi-chat"></i> Commentaires</span>
                        <span class="fw-bold">{{ $novel->comments->count() }}</span>
                    </div>
                </div>
                ...
                <div class="stats-card">
                    ...
                    <div class="mb-2">
                        ...
                        <span>{{ $novel->published_at ? $novel->published_at->format('d/m/Y H:i') : 'Non publié' }}</span>
                    </div>
                    <div class="mb-2">
                        <small class="text-muted">Dernière mise à jour</small><br>
                        <span>{{ $novel->updated_at->format('d/m/Y H:i') }}</span>
                    </div>
                    <div>
                        <small class="text-muted">Créé le</small><br>
                        <span>{{ $novel->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-8">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    @php
                        ...
                        $status = $statusMap[$novel->status] ?? ['text' => $novel->status, 'class' => 'secondary'];
                    @endphp
                ...
                <div class="description-box mb-4">
                    ...
                    <p class="mb-0">{{ $novel->description }}</p>
                </div>
                ...
                @if($novel->chapters->count() > 0)
                    <div class="mb-4">
                        <p class="info-label mb-2">
                            Chapitres ({{ $novel->chapters->count() }})
                        </p>
                            ...
                            @foreach($novel->chapters->sortBy('chapter_number') as $chapter)
                                <div class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <a href="{{ route('authordashboard.novels.chapters.edit', $chapter) }}" 
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Êtes-vous sûr de vouloir supprimer ce roman ?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                    <form action="{{ route('authordashboard.novels.destroy', $novel) }}" method="POST" class="d-inline">
                        @csrf
                        @method('put')
