...
@section('title', 'Modifier le roman')
@section('styles')
    ...
    <div class="container py-4">
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('authordashboard.novels.show', $novel) }}" class="text-decoration-none">{{ $novel->title }}</a></li>
        ...
        <div class="form-container">
            <h1 class="mb-4">Modifier {{ $novel->title }}</h1>

            <form action="{{ route('authordashboard.novels.update', $novel) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-8">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" 
                                   id="title" name="title" value="{{ old('title', $novel->title) }}" required>
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      id="description" name="description" rows="6" required>{{ old('description', $novel->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                <option value="en_cours" {{ $novel->status === 'en_cours' ? 'selected' : '' }}>En cours</option>
                                <option value="terminé" {{ $novel->status === 'terminé' ? 'selected' : '' }}>Terminé</option>
                                <option value="en_pause" {{ $novel->status === 'en_pause' ? 'selected' : '' }}>En pause</option>
                                <option value="annulé" {{ $novel->status === 'annulé' ? 'selected' : '' }}>Abandonné</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            ...
                            <select class="form-select select2 @error('genres') is-invalid @enderror" 
                                ...
                                @foreach($genres as $genre)
                                    <option value="{{ $genre->id }}" 
                                        {{ in_array($genre->id, old('genres', $novel->genres->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $genre->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select select2 @error('tags') is-invalid @enderror" 
                                ...
                                @foreach($tags as $tag)
                                    <option value="{{ $tag->id }}" 
                                        {{ in_array($tag->id, old('tags', $novel->tags->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $tag->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            <div class="image-preview-container" id="imagePreviewContainer">
                                ...
                                <img id="preview" src="{{ url($novel->cover_image) }}" 
                ...
                <div class="d-flex justify-content-end gap-2 mt-4">
                    <a href="{{ route('authordashboard.novels.show', $novel) }}" class="btn btn-secondary">Annuler</a>
